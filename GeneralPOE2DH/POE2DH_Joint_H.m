function [theta, d, alpha, a, h]=POE2DH_Joint_H (xi)
%POE2DH_Joint_H Conversion from a joint twist to DH parameters
%   
%   [theta, d, alpha, a, h]=POE2DH_Joint_H (xi)
%   xi:                     Joint twist, 6 x 1
%   theta, d, alpha, a, h:     DH parameters
    ...
    if abs(norm(w)-1)>DELTA

        error('w should be normalized to a unit vector.')

    end

    if norm(w)~=0 %revolute joint or helical joint

        if abs(w'*v)<DELTA %revolute joint
